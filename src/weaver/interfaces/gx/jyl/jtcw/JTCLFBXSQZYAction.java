...
public class JTCLFBXSQZYAction extends BaseBean implements Action {
    ...
    public String execute(RequestInfo request) {
        ...
        if (!"submit".equals(src)) {
            new weaver.general.BaseBean().writeLog("集团-差旅费报销占用接口退回操作，不执行接口.");
            return SUCCESS;
        }
        //公司代码-值
        String gsdm_value = "";
        ...
        if(("1".equals(bxfs_value)) || ("0".equals(bxfs_value) && "0".equals(sbje_value)) || ("0".equals(bxfs_value) && Double.parseDouble(sbje_value) > 0)){
            ...
            writeLog("集团-差旅费报销占用传入xml参数：" + xmlstring);
            ...
            try {
                ...
                writeLog("集团-差旅费报销占用返回消息：" + returnmessage);
                ...
                if (null != map && !map.isEmpty()) {
                    ...
                    if (!"E".equalsIgnoreCase(type)) {
                        publicmethod.updateJTCLFFYBXZY(requestid, type, code, message,no);
                    } else {
                        publicmethod.setFailMessage(request, "failed", "集团-差旅费报销占用失败：TYPE：" + type + " code：" + code + " zjpzh：" + no + " message：" + message);
                        return SUCCESS;
                    }
                }
            } catch (RemoteException e) {
                publicmethod.setFailMessage(request, "failed", "集团-差旅费报销占用接口异常：" + e);
                return SUCCESS;
            }
        }
        return SUCCESS;
    ...
    public static void main(String[] args) {
    	CWPublicMethod publicmethod = new CWPublicMethod();
		//<?xml version="1.0" encoding="UTF-8"?><ROOT><BUKRS>1000</BUKRS><GJAHR>2017</GJAHR><MONAT>2</MONAT><KOSTL></KOSTL><APPLYNO_EX>CLFBX20170217001</APPLYNO_EX><DMBTR>11</DMBTR><BUDG_CODE>CO019999</BUDG_CODE><ABSTRACT>1111111111</ABSTRACT><YS_TYPE></YS_TYPE></ROOT>
    	SI_1030_ALL2ERP_ZJYSZY_OUTProxy proxy = new SI_1030_ALL2ERP_ZJYSZY_OUTProxy();
        DT_1030_ALL2ERP_ZJYSZY DT_1030_ALL2ERP_ZJYSZY = new DT_1030_ALL2ERP_ZJYSZY();
        DT_1030_ALL2ERP_ZJYSZY.setOUTPUT("<?xml version=\"1.0\" encoding=\"UTF-8\"?><ROOT><BUKRS>1000</BUKRS><GJAHR>2017</GJAHR><MONAT>2</MONAT><KOSTL></KOSTL><APPLYNO_EX>CLFBX20170217001</APPLYNO_EX><DMBTR>11</DMBTR><BUDG_CODE>CO019999</BUDG_CODE><ABSTRACT>1111111111</ABSTRACT><YS_TYPE></YS_TYPE></ROOT>");
        DT_1030_ALL2ERP_ZJYSZY.setSYSTEM("1");
        DT_1030_ALL2ERP_ZJYSZY_RETURN DT_1030_ALL2ERP_ZJYSZY_RETURN = null;
        try {
            DT_1030_ALL2ERP_ZJYSZY_RETURN = proxy.SI_1030_ALL2ERP_ZJYSZY_OUT(DT_1030_ALL2ERP_ZJYSZY);
            String returnmessage = DT_1030_ALL2ERP_ZJYSZY_RETURN.getINPUT();
            System.out.println("集团-差旅费报销占用返回消息：" + returnmessage);
            Map<String, String> map = publicmethod.readXMLForZY(returnmessage);
            if (null != map && !map.isEmpty()) {
                String type = (String) map.get("TYPE");
                String no = (String) map.get("ZJPZH");
