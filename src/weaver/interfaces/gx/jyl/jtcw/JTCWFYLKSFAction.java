...
public class JTCWFYLKSFAction extends BaseBean implements Action {
    ...
    public String execute(RequestInfo request) {
        ...
        writeLog("集团财务-费用借款-释放传入xml参数：" + xmlstring);
        ...
        try {
            ...
            writeLog("集团财务-费用借款-释放返回消息：" + returnmessage);
            ...
            if (null != map && !map.isEmpty()) {
                ...
                if ("S".equalsIgnoreCase(type)) {
                    publicmethod.updateJTCWFYJKSF(requestid, type, code, message);
                } else {
                    publicmethod.setFailMessage(request, "failed", "集团财务-费用借款-释放失败：TYPE：" + type + " code：" + code + " message：" + message);
                    return SUCCESS;
                }
            }
        } catch (RemoteException e) {
            publicmethod.setFailMessage(request, "failed", "集团财务-费用借款-释放接口异常：" + e);
            return SUCCESS;
        }
        return SUCCESS;
    }
