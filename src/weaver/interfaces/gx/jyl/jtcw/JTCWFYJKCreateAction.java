...
public class JTCWFYJKCreateAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		if (!"submit".equals(src)) {
			new weaver.general.BaseBean().writeLog("集团财务-费用借款-创建退回操作，不执行接口.");
			return SUCCESS;
		}
		// 单据类型-值
		String djlx_value = "";
		...
		if (zflb_value.equals("0")) {
			xmlstring = addElement(xmlstring, "LIFNR", skrbm_value);
		} else {
		...
		writeLog("集团财务-费用借款-创建傳入xml参数：" + xmlstring);
		...
		try {
			...
			writeLog("集团财务-费用借款-创建返回消息：" + returnmessage);
			...
			if (null != map && !map.isEmpty()) {
				...
				if ("S".equalsIgnoreCase(type)) {
					...
					publicmethod.setFailMessage(request, "failed",
							"集团财务-费用借款-创建失败：TYPE：" + type + " code：" + code
									+ " applyno：" + no + " message：" + message);
					return SUCCESS;
				}
			}
		} catch (RemoteException e) {
			publicmethod.setFailMessage(request, "failed", "集团财务-费用借款-创建接口异常："
					+ e);
			return SUCCESS;
		}
		return SUCCESS;
